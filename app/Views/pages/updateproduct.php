...
<section class="single_product_details_area d-flex align-items-center">
    ...
    <div class="single_product_desc clearfix">
        ...
        <!-- Form -->
        <form class="cart-form clearfix" action="updatecart" method="post">
            <!-- Select Box -->
            <div style="height: 40px;" class="d-flex mt-50 mb-30">
                <?php foreach ($items as $item) {
                    if ($product['product_name'] == $item['name']) {
                ?>
                        <input type="number" min="1" max="500" id="quantity" onchange="CheckQuantity()" value="<?php echo $item['quantity']; ?>" style="text-align:center" name="quantity">
                <?php    }
                }
                ?>
            ...
            <div class="cart-fav-box d-flex align-items-center">
                <!-- Cart -->
                <button type="submit" name="addtocart" class="btn essence-btn">Update to cart</button>
                <!-- Favourite -->
...
<!-- <script>
    function CheckQuantity() {
        var effect = document.getElementById('quantity');

        var qty = effect.value;
        $.ajax({
            url: "<?php echo base_url() . "/productdetail/update_quantity" ?>",
            method: "POST",
            data: {
                quantity: qty
            },
            success: function(res) {
                if (res > <?php echo $product['product_quantity']; ?>) {
                    $("#wanring").css("display","block");
                    setTimeout(function() {
                        $("#wanring").css("display","none");
                    }, 4000);
